...
//                        fileName = $"{fileMove.Ancla1}{fileMove.Ancla2}{fileMove.Ancla3}_{fileMove.Ancla4}{Path.GetExtension(fileMove.Nombre)}"
//                    };
//                }
...
//                var nomFile = $"{fileMove.Ancla1}{fileMove.Ancla2}{fileMove.Ancla3}_{fileMove.Ancla4}{extension}";
...
//                        status = "Warning",
//                        message = "Los parametros estan vacios.",
//                        fileName = $"{fileMove.FolderPath}{fileMove.FileName}{fileMove.FolderPath}"
//                    };
//                }
...
//                fileName = $"{fileMove.FileName}{fileMove.FolderPath}{fileMove.FileName}_{fileMove.FolderPath}"
//            };
//        }
//        catch (Exception ex)
//        {
    ...
    public class FileMoverRepository : IFileMoverRepository
    {
        ...
        public async Task<FileResultModels> PostMoverDataArchivo(FileMoveModels fileMove)
        {
            try
            {
                ...
                string nomFile = $"{fileMove.Ancla1}{fileMove.Ancla2}{fileMove.Ancla3}_{fileMove.Ancla4}{extension}";
        ...
        private void AsegurarEstructuraDirectorios(string ruta)
        {
            if (!string.IsNullOrEmpty(ruta) && !Directory.Exists(ruta)) Directory.CreateDirectory(ruta);
        }
        ...
        private string PrepararContenedorHtml(string htmlFragmento, bool incluirScriptPaginas = false)
        {
            string scriptPaginacion = incluirScriptPaginas ? @"<script>function subst() { ... }</script>" : ""; // Simplificado para brevedad
            string bodyAttr = incluirScriptPaginas ? "onload='subst()'" : "";

            return $@"<!DOCTYPE html><html><head><meta charset='utf-8'>{scriptPaginacion}</head><body {bodyAttr}>{htmlFragmento}</body></html>";
        }
        ...
        public async Task<FilePdfResultsModel> GetGenerarGuardarPdfs(List<FilePdfADIPdiModel> requests, string rutaFinal)
        {
            ...
            try
            {
                ...
                foreach (var request in requests)
                {
                    ...
                    {
                        string path = GenerarGuardarPdf(request);
                        result.generadosCorrectament++;
                        result.detalleGenerados.Add(request.FileName);
                        ((List<string>)result.detalleNoGenerados).Add(request.FileName);
                    }
                    catch (Exception ex)
                    {
                        result.noGenerados++;
        ...
        public async Task<FilePdfResultsModel> GetGenerarGuardarPdfsADI(List<FilePdfADIPdiModel> requests, string rutaFinal)
        {
            ...
            try
            {
                ...
                foreach (var request in requests)
                {
                    try
                    {
                        request.BodyHtml = _progEvaluacionRepository.GeneradorPDFADIByEvaluacionId(request.EvaluacionId);
                        string path = GenerarGuardarPdf(request);
                        result.generadosCorrectament++;
                        result.detalleGenerados.Add(request.FileName);
                        ((List<string>)result.detalleNoGenerados).Add(request.FileName);
                ...
                var doc = new HtmlToPdfDocument()
                {
                    ...
                    Objects = {
                        new ObjectSettings
                        {
                            HtmlContent = request.BodyHtml,
                            WebSettings = { DefaultEncoding = "utf-8", Background = true },
                            HeaderSettings = { HtmlUrl = !string.IsNullOrEmpty(request.HeaderHtml) ? new Uri(headerTempPath).AbsoluteUri : null, Spacing = 10 },
                            FooterSettings = { HtmlUrl = !string.IsNullOrEmpty(request.FooterHtml) ? new Uri(footerTempPath).AbsoluteUri : null, Spacing = 10 }
                        }
                    }
                };
