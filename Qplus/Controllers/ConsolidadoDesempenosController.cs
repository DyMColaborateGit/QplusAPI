...
namespace Qplus.Controllers
{
    ...
    public class ConsolidadoDesempenosController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [HttpGet("GetListaConsolidadosByEvaluacionId/{EvaluacionId}")]
        public async Task<GetResponse<List<TBL_com_ConsolidadoDesempenoModels>>> GetListaConsolidadosByEvaluacionId(int EvaluacionId)
        {
            GetResponse<List<TBL_com_ConsolidadoDesempenoModels>> resultado = new GetResponse<List<TBL_com_ConsolidadoDesempenoModels>>();
            try
            {
                resultado.Data = await _consolidadoDesempenoService.GetListaConsolidadosByEvaluacionId(EvaluacionId);
                resultado.StatusCode = (int)HttpCodes.OK;
                resultado.Message = new HttpCodesMessage().OK;
                return resultado;
            }
